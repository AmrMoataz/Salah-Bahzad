...
- **Security hygiene (remediates the legacy app), every phase:**
  - TLS validation is **never** disabled (`NFR-APP-SEC-002`) — `ApiClient` never sets `badCertificateCallback`. Dev trusts the Aspire cert via the OS store.
...
- Base URL / app version / portal URL come from `--dart-define` (`API_BASE_URL`, `APP_VERSION`, `PORTAL_URL`) with dev defaults in `AppConfig`. Run/build with e.g. `--dart-define=API_BASE_URL=https://localhost:5010`.
...
- **Firebase:** `firebase_core`/`firebase_auth` build on all four platforms. With no project configured yet (A0), `Firebase.initializeApp()` is guarded in `main.dart` so the app still launches; a real project + `firebase_options.dart` lands in the wiring stream. `google_sign_in` ships only for Android/iOS/macOS — `AppPlatform.googleSignInSupported` hides the button on Windows.
